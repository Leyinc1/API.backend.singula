...
namespace API.backend.singula.Controllers
{
    ...
    public class RolesSistemaController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Create(RolesSistemaDto dto)
        {
            var created = await _service.CreateAsync(dto);
            return CreatedAtAction(nameof(Get), new { id = created.IdRolSistema }, created);
        }

        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, RolesSistemaDto dto)
        {
