...
namespace API.backend.singula.Controllers
{
    ...
    public class PersonalsController : ControllerBase
    {
        ...
        public PersonalsController(IPersonalService service)
        {
            _service = service;
        }
        ...
        public async Task<IActionResult> Create(PersonalDto dto)
        {
            ...
            return CreatedAtAction(nameof(Get), new { id = created.IdPersonal }, created);
        }

        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, PersonalDto dto)
        {
